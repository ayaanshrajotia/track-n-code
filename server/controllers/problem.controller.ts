...
export const getAllProblemsHandler = async (req: NextRequest) => {
    try {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const user: any = req.user;
        const problems = await Problem.find({ user_id: user._id });
        ...
        console.error("Error in getProblemsHandler: ", error);
        return NextResponse.json(
            {
                success: false,
                result: null,
...
export const getSingleProblemHandler = async (
    req: NextRequest,
...
export const deleteProblemHandler = async (
    req: NextRequest,
    { params }: { params: { problem_id: string } }
) => {
    try {
    ...
    try {
        ...
        await Problem.updateOne(
            { problem_id: problem_id, user_id: user._id },
            {
                ...
                time_complexity:
                    data.time_complexity || existingProblem.time_complexity,
                space_complexity:
                    data.space_complexity || existingProblem.space_complexity,
                platform_name:
                    data.platform_name || existingProblem.platform_name,
